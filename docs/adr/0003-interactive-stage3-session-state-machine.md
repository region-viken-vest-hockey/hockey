...
- During migration, `Stage3SessionStore` migrates the existing side files into a session on first load and projects a pending session back onto the legacy file, so existing work directories keep resuming. New capabilities must not add a feature-specific `*_state.json` authority; an architecture test blocks it.
