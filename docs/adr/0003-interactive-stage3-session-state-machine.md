...
The session also owns which pipeline-stage number a transport must answer the pending decision with (`Stage3Session.pending_resume_stage`, exposed as `pending_decision.resume_from` by `stage3 session`): in-Stage-3 sub-decisions (`shared_host_assignment`, `arena_conflict_resolution`) with `--resume-from 3`, and post-plan candidate decisions (adoption/repair/search) with `--resume-from 4`, even though applying the latter may loop back into Stage 3 internally. The CLI rejects an action submitted with the wrong resume number with a precise lifecycle error before building a stage context or invoking any domain capability, so no harness adapter has to duplicate the capability table.
