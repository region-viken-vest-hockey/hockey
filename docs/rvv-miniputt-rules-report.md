...
| Verifikatorens uløste vertskapskrav er autoritative ved eksport (issue #274) | Rett før Stage 4-eksport kjøres den kanoniske `planning_contract.verify_candidate` på den endelige planen på nytt, og resultatet overskriver planens `unresolved_hosting_obligations`/`unresolved_external_conflicts`/`unresolved_participation_shortfalls` før `manual_schedule.html` rendres. Dette hindrer at et krav verifikatoren finner uløst (f.eks. etter en sen optimerings- eller beslutningsrunde) forblir usynlig i den publiserte manuelle visningen. |
