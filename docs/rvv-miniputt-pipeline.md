...
Each source is graded by one deterministic, planner-independent integrity verdict (`tournament_scheduler.pipeline.source_integrity`): `complete`, `suspicious`, `partial` or `failed`. A suspicious event-count regression versus the previous scrape, a duplicate-heavy or fabricating scraper fingerprint, a partial/swallowed navigation failure, a blocked/skipped/errored scrape, or an unproven browser week/month navigation all fail the gate. Stage 2 folds a suspicious or partial source into the explicit `source_review_required` calendar tier (never `known`), and `refresh_calendars` persists the per-club `club_source_integrity` verdict and `club_coverage_proven` proof into the verification problem. A `source_review_required` club is treated exactly like an untrusted one by the assessment: its rows cannot become actionable proposals, and source absence is never a negative occupancy claim. The coarse minimum-count expectation alone is deliberately not used as a hard rejection: counts are reported before/after for review.
