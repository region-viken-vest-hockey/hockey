...
### Booking evidence crosswalk

`season booking-assessment` is the read-only, deterministic assessment boundary for matching scraped host calendars to canonical tournaments. It is a pure projection over the current canonical `schedule.json` and `decisions.json` plus the stored verification-context calendar: it never writes canonical state, never confirms an association and never claims that calendar absence proves a tournament is unbooked. The report is bound to the exact `canonical_state_revision` and to each club's `calendar_fingerprint`, so a second agent rerunning it against the same frozen inputs reproduces the same classifications; any schedule, decision or calendar change must be followed by a fresh run rather than trusting a cached result.

Each source is graded by one deterministic, planner-independent integrity verdict (`tournament_scheduler.pipeline.source_integrity`): `complete`, `suspicious`, `partial` or `failed`. A suspicious event-count regression versus the previous scrape (only compared when that previous scrape used the same requested window, so a shorter refresh is not data loss), a duplicate-heavy or fabricating scraper fingerprint, a partial/swallowed navigation failure, a blocked/skipped/errored scrape, or an unproven browser week/month navigation all fail the gate. Stage 2 folds a suspicious/partial source, or any non-exempt source without a coverage proof, into the explicit `source_review_required` calendar tier (never `known`), and `refresh_calendars` persists the per-club `club_source_integrity` verdict and `club_coverage_proven` proof into the verification problem. A `source_review_required` club is treated exactly like an untrusted one by the assessment: its rows cannot become actionable proposals, and source absence is never a negative occupancy claim. An operator-confirmed club is deliberately different: the confirmation is out-of-band placement authority and keeps `known`, but it never makes the source coverage-proven and must not be read as negative booking evidence. The coarse minimum-count expectation alone is deliberately not used as a hard rejection: counts are reported before/after for review.

`refresh_calendars` always rescrapes every configured source with the cache bypassed (`force_refresh=True`), so a stale-cache result can never masquerade as a fresh refresh. In addition (issue #467), every refresh independently classifies each tournament currently hosted at Kongsberg and Ringerike against that same fresh scrape and returns read-only `planned_tournament_reconciliation` evidence: a `clubs` map (keyed by club, with the per-tournament `classified` rows and a `requires_review_count`) plus an `assessment` block. The per-club classification reuses the identical rules as `season reconcile-calendar-bookings` (`confirmed_booked`, `ambiguous`, `not_checkable`), but a row is still pulled into `requires_review_count` even when it is `confirmed_booked` if either concern below applies, so a valid association can never look fully green on its own:

- its active calendar association conflicts with a later active manual `manually_not_booked` assertion (#454) -- flagged `manual_conflict: true`;
- the association predates this refresh and the club's calendar status has since degraded to `source_review_required`/untrusted -- flagged `source_integrity_concern: true` (with `source_status` recording the degraded value). The booking authority itself is preserved (a pre-existing association is not invalidated by a later bad scrape), but the degraded source stays visible as its own concern rather than being silently absorbed into a green row.

Because the raw overlap classifier alone cannot distinguish a truly absent booking from one that moved to a non-overlapping time/date, the `assessment` block reuses the same read-only `booking_assessment()` crosswalk (scoped to the clubs actually reconciled) so a shifted slot, competing candidate or unmatched event stays visible next to the simple classification instead of collapsing into the same `no_covering_event_for_current_slot` reason as a real absence. `booking_assessment()` binds itself to one exact `canonical_state_revision`, so persisting it inside the refresh's own committed `calendar_evidence` would be circular (the commit recomputes the canonical revision from that very content, immediately invalidating the binding it just wrote). Only the deterministic per-club `clubs` classification is persisted into `calendar_evidence`; the `assessment` is computed separately against whichever revision is actually current once the call returns -- the freshly committed revision for a live refresh, or the unchanged `before_revision` for a `--dry-run` preview -- and is returned to the caller without being written to `schedule.json`. This closes the specific gap where a refresh could report success and an up-to-date calendar fingerprint while a tournament placed at one of those clubs during planning silently stopped showing up in the source calendar. The reconciliation is strictly evidence: it is bound to the same fetch and never writes a booking decision or approval itself -- an operator/harness still confirms a match through `season confirm-calendar-booking` or persists a classification through `season reconcile-calendar-bookings`.

It emits a machine-readable JSON object (the default text view summarises it) with a per-club source block (status, `source_trust`, `source_review_required`, `source_integrity`, `coverage_proven`, event count, fingerprint, and an explicit `trusted_for_negative_claim: false`) and two crosswalks:
...
Each tournament is classified `associated`, `manually_asserted`, `proposed_unchanged`, `proposed_changed_slot`, `competing_candidates`, `ambiguous`, `unmatched` or `not_checkable`. A candidate carries evidence and counterevidence (including an age-group token mismatch in the title or an arena mismatch), plus `source_trusted`/`actionable` flags: an observation from an untrusted source stays visible but can never be an actionable proposal, and contradicting title/arena evidence marks it non-actionable. Only actionable candidates may contest or resolve a proposal, so one exact booking is still reported `proposed_unchanged` while an unrelated conflicting observation stays visible as counterevidence; when only conflicting observations remain the row is `ambiguous`. Every `proposal_is_binding` is `false`. `authority` (a deliberate calendar association or manual assertion) is reported separately from `calendar_source_checkable`, so an explicit authority is never silently demoted by an untrusted scrape. When one event plausibly belongs to several tournaments, or several plausible events map to one tournament, or a source is missing/untrusted, the row stays `competing_candidates`/`ambiguous`/`unmatched`/`not_checkable` rather than resolving the ambiguity. `season calendar-booking-findings`, `season booking-status` and the explicit `season confirm-calendar-booking` / `season booking-set` boundaries remain the only persistence paths; a `proposed_*` row is a claim to review, not an approval.
...
Publication builds a separate allowlisted/privacy-checked public bundle and promotes the exact source export lifecycle from `draft` to `published`. A successful publication also records the immutable published baseline and seals the season (or appends a publication revision for an already sealed season); the publication boundary refuses unexplained canonical drift and a stale export of a sealed schedule. Replacing the public snapshot of an already published season is a first-class workflow: resolve the current published run from authoritative publication history (never from an arbitrary export directory or the moving `latest/` path) with `season lifecycle --json` / `season publication-evidence --json` / `operator publish-history --json`, regenerate only with `season export`, and inspect the exact stable-id published→canonical delta before publishing. Before replacing `/latest/`, the boundary verifies the previous bundle is retained behind an immutable reference (Pages run id / commit / bundle fingerprint) and that its `/runs/<run_id>/` snapshot exists; a legacy baseline without such a reference fails closed and must be backfilled with `season seal-published`. The new immutable baseline links the previous publication and records the exact republish delta plus the separately-reported approval/booking/protection changes; the before/after record is retained under `season/<season>/evidence/publications/<publication_id>/`. Rollback remains explicit through publication history, and a failed publication never modifies the published output. The shared agent procedure is `.agents/commands/rvv-miniputt/republish.md`.
