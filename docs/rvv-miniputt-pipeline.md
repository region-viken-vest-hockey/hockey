...
`refresh_calendars` always rescrapes every configured source with the cache bypassed (`force_refresh=True`), so a stale-cache result can never masquerade as a fresh refresh. In addition (issue #467), every refresh independently classifies each tournament currently hosted at Kongsberg and Ringerike against that same fresh scrape and records the result as read-only `planned_tournament_reconciliation` evidence (keyed by club, with the per-tournament `classified` rows and a `requires_review_count`), reusing the identical classification rules as `season reconcile-calendar-bookings` (`confirmed_booked`, `ambiguous`, `not_checkable`). This closes the specific gap where a refresh could report success and an up-to-date calendar fingerprint while a tournament placed at one of those clubs during planning silently stopped showing up in the source calendar. The reconciliation is strictly evidence: it is bound to the same fetch and never writes a booking decision or approval itself -- an operator/harness still confirms a match through `season confirm-calendar-booking` or persists a classification through `season reconcile-calendar-bookings`.
