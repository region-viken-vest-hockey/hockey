...
`refresh_calendars` always rescrapes every configured source with the cache bypassed (`force_refresh=True`), so a stale-cache result can never masquerade as a fresh refresh. In addition (issue #467), every refresh independently classifies each tournament currently hosted at Kongsberg and Ringerike against that same fresh scrape and records the result as read-only `planned_tournament_reconciliation` evidence: a `clubs` map (keyed by club, with the per-tournament `classified` rows and a `requires_review_count`) plus an `assessment` block. The per-club classification reuses the identical rules as `season reconcile-calendar-bookings` (`confirmed_booked`, `ambiguous`, `not_checkable`), but a `confirmed_booked` row whose active calendar association conflicts with a later active manual `manually_not_booked` assertion (#454) is flagged `manual_conflict: true` and still counts toward `requires_review_count` -- a valid association must never look green when the club has since said the slot was released. Because the raw overlap classifier alone cannot distinguish a truly absent booking from one that moved to a non-overlapping time/date, the `assessment` block reuses the same read-only `booking_assessment()` crosswalk (scoped to the clubs actually reconciled) so a shifted slot, competing candidate or unmatched event stays visible next to the simple classification instead of collapsing into the same `no_covering_event_for_current_slot` reason as a real absence. This closes the specific gap where a refresh could report success and an up-to-date calendar fingerprint while a tournament placed at one of those clubs during planning silently stopped showing up in the source calendar. The reconciliation is strictly evidence: it is bound to the same fetch and never writes a booking decision or approval itself -- an operator/harness still confirms a match through `season confirm-calendar-booking` or persists a classification through `season reconcile-calendar-bookings`.
