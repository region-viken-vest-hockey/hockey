...
- a normal Stage 3 run whose planning window matches a promoted season resolves the canonical files directly and adopts the published schedule as its baseline candidate, preserving durable tournament ids, order and placements instead of regenerating a wholly new season. This does not depend on `.pipeline`; the Stage 3 checkpoint, mid-planning decision problems, refinement and the pre-export hard-verification gate all resolve the same canonical baseline;
- `placement_locked`/approved tournaments are pinned and their dates are locked, and a candidate that moves or drops one fails hard verification (`canonical_placement_locked`, `canonical_participants_locked`, `canonical_locked_tournament_missing`) before any canonical write. The local-search engine also excludes placement-locked slots from date/host/start-time moves and participant-locked slots from team swaps, so the search never proposes a move the baseline forbids;
...
Tournament IDs are durable lifecycle identifiers, not schedule placement keys. Stage 3 assigns an opaque ID when a new logical tournament is created and all ordinary mutations/search moves (date, time, arena, host and participant edits) preserve that ID. Replacements that create a new logical tournament carry explicit lineage (`derived_from`) and the serialized plan carries an identity registry so final verification can reject missing/duplicate active IDs and unknown manual/waiver/lineage references instead of silently regenerating identities.

`SeasonPlan` checkpoint serialization is owned by the public versioned codec in `tournament_scheduler.serialization.season_plan`. Its default v1 payload preserves the historical Stage 3 plan keys for checkpoint/fingerprint compatibility, while standalone consumers can request an explicit `schema_version` field and older unversioned v1 checkpoints continue to load.
