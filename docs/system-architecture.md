...
| Season holiday-date exceptions | canonical `canonical_holiday_exceptions` record + projection into the existing `date_policy` read path | one durable record per exact date with reason/actor/timestamp provenance; removes only the derived holiday-policy exclusion for that date and never an explicit canonical banned date; recording/removing is a decision-only write that advances the canonical-state revision without moving tournaments |
...
| Feasible repair/action enumeration | canonical application/decision capability | common `local_repair_options` boundary over small planner-neutral providers: `rehost_tournament`/`remove_tournament`/participant repair for `host_team_missing`, `fill_participant`/`swap_participant` for an underfilled roster, `move_same_host_date`/`move_same_host_start_time`/`swap_compatible_tournament_placement`/`interpret_calendar_event_as_movable` for a manual placement, same-age rehost for a hosting-balance/coverage deficit (`hosting_balance_repair`), `replace_participants` roster substitution that keeps an already-valid placement for a double-booked/duplicated participant (`placement_preserving_roster_repair`), bounded participant/host search for a participation strong-goal deviation (`participation_deviation_repair`), `move_to_movable_capacity`/`move_to_movable_capacity_reselect_participants` for verified host-controlled ice, ranked conflict-aware manual candidate weekends (`candidate_weekends`) as read-only evidence, materialization of a genuine unplaced obligation into a verified tournament (`unplaced_placement_repair`) with its coupled capacity-release/cross-age dimensions and per-finding search coverage, bounded cross-age coupled placement + same-age roster repair for a `temporal_clustering` finding (`coupled_placement_repair`, including a same-club/same-age sibling substitution when a hard-valid exchange would otherwise materially regress a multi-team club's individual team), bounded same-club sibling rotation / coupled home + away swaps that rebalance a multi-team club's intra-club home representation without moving host/date/arena (`home_representation_repair`), bounded same-club/same-age sibling substitutions that redistribute an aggregate-complete but label-uneven club participation pool without moving date/host/arena/slot or touching guest reservations (`intra_club_distribution_repair`), bounded neighborhood search (`search_neighborhood_repair`) for a locally searchable hard finding no direct option repairs, batch relocation of tournaments off canonical excluded dates (`date_policy_relocation`), manual-placement fallback |
...
| State mutation and persistence | canonical application/season operation | atomically apply validated changes while respecting locks/approvals; on a sealed season a scoped mutation is authorized only by the application layer re-running the exact typed operation (move, swap/replace participant, bounded repair option) at the apply boundary and requiring the submitted candidate to equal the reproduced result as a whole plan (excluding only reconciliation-owned derived/reporting projections) -- a caller-built contract, mode string, affected-id list or capability object is never authorization, and unrelated obligations, games or placement/host-confirmation metadata may not drift; one-tournament roster substitution (`CanonicalSeasonService.replace_participant`, CLI `season replace-participant`) for replacing one participant with a registered same-age team without moving placement/host; compose several explicitly scoped mutations against one in-memory candidate and commit once (`CanonicalSeasonService.batch_maintenance`, CLI `season batch`) so simultaneous pre-existing request-constraint violations can be repaired together without weakening the single-mutation full-season constraint gate |
...
`tournament_scheduler/rule_catalog.py` is the one searchable, metadata-only registry of every active scheduling semantic: its stable ID, primary classification (`hard_constraint`, `operational_obligation`, `soft_objective`, `operator_decision`, `fact_evidence_semantic`), meaning, canonical owner/facade, input source, verifier/measurement owner, mutation/search providers, evidence projection, tests, status and the small amount of `precedes`/`depends_on` precedence that actually matters. The generated agent-facing table lives in [`architecture/rule-catalog.md`](architecture/rule-catalog.md) and is kept current by `scripts/render-rule-catalog.py`.

The catalog is navigation/identity only -- it never verifies, scores or generates rule code; the deterministic implementations it names remain the executable truth. The canonical verifier annotates each returned violation with its stable `rule_id`, the season-findings projector attaches each finding's `rule_id` (plus a `counts_by_rule_id` summary), quality/score metrics carry an `objective_id` for every registered score path, and the per-run Rules ("Regler") model attaches the matching `catalog_id` plus the catalog's classification/meaning/owner. The Regler HTML and rules report also render a reference section generated directly from `rule_catalog.active_catalog_references()`, so the surfaces consume one identity instead of maintaining a second semantic list. `tests/test_rule_catalog.py` guards against duplicate IDs, unregistered public verifier codes or season finding codes, unregistered quality score paths, ambiguous finding-code ownership, invalid precedence references, owner/test drift and a stale generated table.

For hosting, the catalog makes three separate semantics with precedence explicit rather than collapsing them into vague "hosting fairness": `hosting_age_group_coverage` (operational obligation owned by `hosting_coverage`'s coverage-floor/target math) precedes `hosting_proportional_balance` (soft objective), and `hosting_responsibility` (cross-path invariant owned by `hosting_responsibility`) keeps an assigned burden with the club that owes it through placement/repair/search. A structural shortfall (fewer tournaments than club x age-group coverage obligations) is surfaced as an unresolved coverage obligation, never hidden inside the soft balance score.
...
Date admissibility is owned by the planner-independent `date_policy` module: it derives the excluded dates (the Monday-Sunday week containing a Norwegian public holiday and the weekend immediately before a holiday) for the planning window, applies any canonical `holiday_date_exceptions` to remove only those derived exclusions for exact dates, and it is the single implementation the initial scheduler's `HolidayConflictChecker`, the normalized `planning_problem` (`date_exclusions` / `holiday_date_exceptions`), every repair/optimizer date move and `verify_candidate` read. The verifier re-derives the exclusions from the planning window itself instead of trusting the planner that produced the candidate, so a candidate a later optimizer/repair moved onto an excluded date fails final verification (`holiday_date_used`) no matter which path created it, and the active excluded dates are surfaced as `holiday_dates_not_used` rules evidence. Explicit operator banned dates are composed separately through `manual_adjustments.banned_dates` and take precedence over a holiday-date exception.
...
`season refresh-calendars` is the canonical maintenance boundary for advancing external availability evidence on an already promoted season. It reruns Stage 2 with the canonical configured sources, replaces only the calendar facts in the promoted verification context, records before/after evidence fingerprints and per-source fetch provenance, re-verifies the unchanged schedule, advances the canonical-state revision, and marks existing export/audit evidence stale. It never moves tournaments, edits rosters, changes approvals/protections/request constraints, or treats approvals as waivers for newly discovered calendar conflicts; those conflicts surface through ordinary verification/findings and are compared against the existing season-quality baseline as NEW/REGRESSED unless the operator later explicitly rebaselines. Separate per-tournament booking evidence records whether a host-calendar review found a matching booking, found no booking, was ambiguous, was not checkable, or has gone stale; this booking status is independent of approval state and is projected into the season-plan cards and heatmap as operational follow-up evidence rather than changing the schedule. Calendar overlap is occupancy evidence, not proof that the interval is this tournament: bulk reconciliation only records `confirmed_not_booked` (trustworthy calendar, no relevant event) and ambiguous candidate evidence for overlapping events, while `confirmed_booked` comes solely from an explicit event-to-tournament association created by `confirm-calendar-booking`. The booking-status projection enforces that same invariant, so a persisted positive evidence record with no currently-valid association (a legacy overlap match, or one whose association was released) is surfaced as requiring review rather than as `confirmed_booked`.

`season reconcile-config` is the matching maintenance boundary for promoted config-owned facts whose semantic contract changed after promotion. It compares the promoted verification-context config with the current canonical input contract, applies only recognized semantic migrations that preserve the already-reviewed operational meaning (for example legacy ice-time values whose old effective occupancy was `stored + 5 * actual_round_count`), re-verifies the unchanged plan under the migrated config, and commits one auditable canonical revision. It does not copy arbitrary current workbook values into a promoted season, does not move tournaments or change rosters/approvals/protections, and refuses persistence when a difference requires replanning/operator intent or the preserved occupancy reveals a real hard conflict.
...
### Accepted quality debt is one season-quality baseline, not a waiver

A promoted operational season usually carries accepted, non-hard deviations that cannot realistically be eliminated (season capacity, arena availability, excluded weekends, club constraints). Re-surfacing them as open work on every maintenance change makes it hard to tell whether a requested change introduced a *new* problem. The **season quality baseline** (`tournament_scheduler.season_baseline`, lifecycle `CanonicalSeasonService.season_baseline_*`) is the one regression reference for that purpose.

It is deliberately separate from every adjacent concept:

- **canonical schedule truth** (`season/<season>/schedule.json`) is the actual placement/roster state; the baseline never edits it;
- **operator waiver** is a narrow, authorized exception to one classified *hard* planning rule;
- **participation acceptance** (`operator_accepted`) is a decision about one bounded participation strong-goal deviation, bound to its scope/magnitude;
- **season baseline** is a full-season comparison over every non-hard finding, recording stable finding ids plus measured severity, not aggregate counts.

The baseline record (persisted in `decisions.json`) stores the season, source canonical-state revision, schedule fingerprint, a finding-context fingerprint, creation time/actor/note, and per finding the stable `finding_id`, `code`, `category`, `rule_id`, measured values and search-coverage state. Comparison classifies each fresh finding as `KNOWN`, `IMPROVED`, `RESOLVED`, `REGRESSED` or `NEW` from its own measurement, so a replaced finding with unchanged counts is still a regression and a participation shortfall going from `-1` to `-2` is `REGRESSED` despite the same id. Hard findings are never baseline-eligible: `season baseline create`/`advance` refuse while hard verification fails, and hard findings remain blocking and visible in the default `season findings` view. `advance` is permitted only when there are no `NEW`/`REGRESSED` findings and records the prior baseline and comparison in `decisions.json` history; a worse/new state requires an explicit replacement decision. Baseline mutations are decision-only writes, so they advance the canonical-state revision but not the schedule fingerprint when no tournament changed, and the active baseline is exposed in canonical export evidence and the audit context.
...
For a promoted season, a host may later confirm that one raw scraped busy event is the actual booking for one canonical RVV tournament even when the event title is semantic rather than an exact tournament id. That is represented as a durable `calendar_booking_associations` overlay in `decisions.json` (`calendar-event fingerprint -> tournament id`), not by editing Stage 2/source evidence and not by adding broad title regexes. `season calendar-booking-candidates` deterministically narrows possible matches by host/date/overlapping interval and scheduled canonical tournament facts; the active harness/operator owns the semantic selection. `season confirm-calendar-booking` validates that the event covers the tournament's current canonical occupied interval, records note/provenance and interval facts, projects the association into the verifier so the event is ignored only for that associated tournament, and then reuses ordinary approval/placement-lock semantics with participant editability preserved. The same event remains a fixed busy conflict for every other tournament. Projection revalidates active associations against current event, tournament host/arena/date/start facts and the recomputed canonical occupied interval, and fails closed: stale evidence remains visible through `calendar-booking-findings`/`season findings`, but it no longer suppresses a calendar conflict. One scraped event fingerprint may have only one active tournament association; deliberate rebinding first releases the old association with `season release-calendar-booking` so the history remains auditable.
