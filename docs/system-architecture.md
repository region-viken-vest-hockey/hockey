...
The catalog is navigation/identity only -- it never verifies, scores or generates rule code; the deterministic implementations it names remain the executable truth. The canonical verifier annotates each returned violation with its stable `rule_id`, and the per-run Rules ("Regler") model attaches the matching `catalog_id`, so reports reuse one identity instead of maintaining a second semantic list. `tests/test_rule_catalog.py` guards against duplicate IDs, unregistered public verifier codes, invalid precedence references, owner/test drift and a stale generated table.
