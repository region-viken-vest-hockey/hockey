...
- export, privacy and publication safety gates, including independent XLSX/HTML artifact-parity and canonical-freshness verification (`tournament_scheduler/pipeline/export_parity/`, `export-parity` CLI, `export_parity.json`, publish preflight) that compares the actual generated bytes by stable tournament id.
...
| State mutation and persistence | canonical application/season operation | atomically apply validated changes while respecting locks/approvals; on a sealed season a scoped mutation is authorized only by the application layer re-running the exact typed operation (move, swap/replace/remove participant, bounded repair option) at the apply boundary and requiring the submitted candidate to equal the reproduced result as a whole plan (excluding only reconciliation-owned derived/reporting projections) -- a caller-built contract, mode string, affected-id list or capability object is never authorization, and unrelated obligations, games or placement/host-confirmation metadata may not drift; one-tournament roster substitution (`CanonicalSeasonService.replace_participant`, CLI `season replace-participant`) for replacing one participant with a registered same-age team without moving placement/host; one-or-more-tournament participant removal with no replacement (`CanonicalSeasonService.remove_participant`, CLI `season remove-participant`) that regenerates the reduced field and, for a genuine season/age-group withdrawal, records a durable revision-bound eligible-pool reduction for the whole age group from the earliest affected tournament (`participation_withdrawals.py`) without rewriting the registered roster or earlier completed tournaments, verified independently by the `withdrawn_team_participating` rule and reversed only by a verified `season release-withdrawal` (which refuses a premature release and can atomically restore the participants) while keeping provenance; compose several explicitly scoped mutations against one in-memory candidate and commit once (`CanonicalSeasonService.batch_maintenance`, CLI `season batch`) so simultaneous pre-existing request-constraint violations can be repaired together without weakening the single-mutation full-season constraint gate |
...
`season refresh-calendars` is the canonical maintenance boundary for advancing external availability evidence on an already promoted season. It reruns Stage 2 with the canonical configured sources, replaces only the calendar facts in the promoted verification context, records before/after evidence fingerprints and per-source fetch provenance, re-verifies the unchanged schedule, advances the canonical-state revision, and marks existing export/audit evidence stale. The promoted source configuration (source identity/URL plus the effective code-owned Stage 2 dispatch engine, trust/classification and coverage facts) is persisted as a source-policy fingerprint; a refresh whose current workbook source configuration differs is refused unless the operator explicitly accepts the source-policy change, so today's workbook can never silently redefine already-promoted evidence. The complete previous calendar payload and source policy are archived content-addressably in the same atomic canonical-state commit as the reference to them, so the replaced evidence is reconstructable even on a season's first refresh after promotion and the reference can never dangle. It never moves tournaments, edits rosters, changes approvals/protections/request constraints, or treats approvals as waivers for newly discovered calendar conflicts; those conflicts surface through ordinary verification/findings and are compared against the existing season-quality baseline as NEW/REGRESSED unless the operator later explicitly rebaselines. Separate per-tournament booking evidence records whether a host-calendar review found a matching booking, found no booking, was ambiguous, was not checkable, or has gone stale; this booking status is independent of approval state and is projected into the season-plan cards and heatmap as operational follow-up evidence rather than changing the schedule. Calendar overlap is occupancy evidence, not proof that the interval is this tournament, and calendar absence at the current canonical slot is not proof that a tournament is unbooked: a pending/approved tournament whose slot has no covering event is recorded as ambiguous (`no_covering_event_for_current_slot`), while `confirmed_booked` comes solely from an explicit event-to-tournament association created by `confirm-calendar-booking`. Only explicit source-supported negative/rejection evidence may assert `confirmed_not_booked`. The booking-status projection enforces both invariants: a persisted positive evidence record with no currently-valid association (a legacy overlap match, or one whose association was released) and a `confirmed_not_booked` record derived only from current-slot absence are surfaced as requiring review rather than as `confirmed_booked`/`confirmed_not_booked`. A separate durable `manual_booking_assertions` overlay in `decisions.json` carries explicit operator/club conclusions for tournaments no calendar can resolve. It is revision-bound to the current canonical occupied interval, has typed `authority=manual_club_confirmation` (or `manual_club_confirmation_interpretation` for a deliberately accepted club-wide interpretation), and projects as `manually_booked`/`manually_not_booked`; a routine `reconcile-calendar-bookings`/`refresh-calendars` never erases or demotes it, a conflicting calendar association surfaces a review conflict rather than overwriting it, independent actionable calendar warnings stay visible, and any material canonical slot change stales it. A stale assertion is replaced directly by re-confirming the new slot with fresh source provenance while the previous record is retained as superseded audit history; a positive confirmation requires a traceable source reference or rationale.
...
Replacing the public snapshot of a sealed season is owned by `pipeline/publication_lifecycle` and `pipeline/publication_evidence`. The new immutable baseline links the publication it replaced and records the exact stable-id republish delta plus separately-reported approval/booking/protection changes; the before/after record is retained under `season/<season>/evidence/publications/<publication_id>/`. Before replacing `/latest/` the boundary verifies the previous bundle is retained behind an immutable Pages reference (`run_id` / commit / bundle fingerprint) and that its `/runs/<run_id>/` snapshot exists, so rollback stays reachable. A legacy baseline without such a reference fails closed and is backfilled with `season seal-published`; the shared agent procedure is `.agents/commands/rvv-miniputt/republish.md`.
