...
1. Run focused regression tests first, then the fast local gate `scripts/check lint quick` before handoff. Add targeted integration/CLI/reproducibility checks for affected contracts; run `scripts/check full` locally only when the scope or risk justifies the full hermetic suite (for example substantial planner/lifecycle changes), not automatically after the fast gate. For source/parser changes, use appropriate hermetic tests and `scripts/check live` when live upstream validation is required and available. The complete `make check` quick/PR gate and scheduled/manual full suite belong to CI or an explicitly requested local verification run; do not repeatedly run both full and quick tiers by default. Run `scripts/check rule-catalog` locally when rule-catalog metadata, rule identities/owners, or generated catalog content changes; otherwise rely on CI's catalog freshness check. `scripts/render-rule-catalog.py` without `--check` is generation, not a routine test action. Record exact commands and outcomes; do not claim unrun checks passed.
