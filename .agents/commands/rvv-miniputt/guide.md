...
- change tournament participants or make room for a team -> inspect `season protections` and `season constraints`; for "replace team A with team B in this tournament" evaluate `season replace-participant --dry-run --request-id <id>`, for a team dropping out with no same-age replacement evaluate `season remove-participant --dry-run --request-id <id>` (use `--reconcile-withdrawal` only for a genuine season/age-group withdrawal, or one atomic `batch` of `remove_participant` operations across several tournaments), and for a true two-tournament exchange evaluate `season swap-participants --dry-run --request-id <id>`. Require acceptable consequences for all affected teams before applying. Prefer the smallest verified roster/capacity repair before moving an otherwise valid placement or broad replanning.
