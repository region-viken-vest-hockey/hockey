...
- **genuine season/age-group withdrawal** (`--reconcile-withdrawal`) additionally records a revision-bound eligible-pool reduction scoped to exactly the affected tournaments, so the verifier and game generator see the correct active pool. The registered `Lag` roster and every historical/completed tournament are never rewritten; the record is additive canonical provenance and is part of the canonical-state revision.
