...
When no calendar can be scraped, matched or trusted (a blocked/empty source, a generic overlap, or an explicit club statement that its bookings are made but not reflected in a public calendar), record the decision as an explicit manual assertion with `season booking-set`. It is durable source authority, not a scrape result: it lives in its own decisions key, is revision-bound to the current canonical slot, and is never erased or demoted by a later `season reconcile-calendar-bookings`/`season refresh-calendars`. It projects as `manually_booked`/`manually_not_booked` with `authority=manual_club_confirmation`, distinct from calendar-verified `confirmed_booked`/`confirmed_not_booked`. A material canonical change (date/start/duration/host/arena) stales the assertion until the operator confirms the new slot again; a later calendar event that contradicts it surfaces a conflict requiring review instead of silently overwriting. Use `--reference` for the actual source (email id/date/sender) and `--source-scope club_wide_interpretation` when the assertion is one deliberate per-tournament interpretation of a club-wide statement rather than a fabricated itemized confirmation. A stated source interval that differs from the canonical occupancy (for example a shorter emailed window) is recorded with `--stated-start`/`--stated-end` as follow-up; the canonical occupancy is never silently shrunk. Repeating the same assertion is idempotent; replacing a different conclusion requires `--supersede` with a reason; `season booking-clear` revokes an incorrect assertion. Rejection never cancels or deletes the tournament; it stays visible as follow-up. **Never regenerate or reopen a published/sealed season to work around a scrape failure** -- this manual fallback exists precisely so a failed source does not require replanning.
