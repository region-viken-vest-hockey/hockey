...
scripts/rvv-miniputt season booking-set --season 2026-2027 --tournament-id <id> --status booked --note "club confirmed by email" --reference "<email id/date/sender>"
scripts/rvv-miniputt season booking-set --season 2026-2027 --tournament-id <id> --status not-booked --note "club rejected the slot"
scripts/rvv-miniputt season booking-clear --season 2026-2027 --tournament-id <id> --note "wrong id"
...
scripts/rvv-miniputt season remove-participant --season 2026-2027 --tournament-id <id> --remove-team "<team>" --reconcile-withdrawal --request-id <request-id> --dry-run
scripts/rvv-miniputt season withdrawals --season 2026-2027 --json
scripts/rvv-miniputt season release-withdrawal --season 2026-2027 --request-id <withdraw-request-id> --note "restored after registration reconciliation"
...
scripts/rvv-miniputt export-parity --export-dir export/2026-09-24T2124 --json
...
- run `season lifecycle --json` (or the pre-publication reconciliation check) before publishing a new revision; a new publication refuses unexplained schedule drift. Publication also refuses a season-plan export whose XLSX/HTML artifact parities are not `PASS` or whose embedded canonical revision is stale relative to canonical state; two artifacts that merely agree with each other are not fresh. Regenerate the export from current canonical state instead of publishing a `FAIL`/`NOT_CHECKABLE` pair (`scripts/rvv-miniputt export-parity --export-dir <dir> --json` inspects an existing pair read-only).
...
When a registered team drops out and there is no same-age replacement to substitute in, use `season remove-participant` instead of `replace-participant` or a whole-season replan. It removes exactly one participant from one or more same-age tournaments, keeps every date/host/arena and booked occupancy interval, regenerates each affected tournament's games through the configured-rounds generator, records a change protection, and runs the same full-season hard-verification/hosting-responsibility/consequence gates as a swap. Distinguish two intents: a **one-tournament participant absence** (the default) changes only that tournament and fails closed if the reduced shape is avoidably underfilled; a genuine **season/age-group withdrawal** additionally passes `--reconcile-withdrawal`, which records a durable, revision-bound eligible-pool reduction for the whole age group (from the earliest affected tournament date) while preserving the registered `Lag` roster and all earlier completed/historical tournaments. Never pass `--reconcile-withdrawal` merely to make an underfilled candidate legal; it is a durable statement that the team has genuinely left the age group. When a withdrawal affects several tournaments, use one atomic `batch` with a `remove_participant` operation per tournament (and `reconcile_withdrawal: true` on each) so the whole withdrawal commits once or not at all. A withdrawal only reduces eligibility while its team remains in the registered pool, but it is **not** silently superseded if a roster regains the team: the verifier reports `withdrawn_team_participating`, so a later maintenance or rebuild must not reintroduce the team. Reverse it explicitly with the verified `season release-withdrawal`: release refuses to write when it would leave the current schedule invalid, and `--restore-participant` performs the atomic, verified restoration (add the team(s) back, regenerate games, release the removal guards and the record) in one commit through the same complete canonical apply boundary (typed/replayable sealed-season mutation, request constraints, locks, guest integrity, operational acceptability, hosting responsibility and published-baseline replay) as every other canonical mutation; inspection is available with `season withdrawals`. Provenance is never deleted.

When promoted-season maintenance is blocked because canonical findings/repair/search are verifying against stale external arena evidence, use `season refresh-calendars` to run a fresh Stage 2 scrape and advance only the calendar evidence. Start with `--dry-run`; if the source change is expected, commit the refresh, then inspect `season findings`. The refresh persists the promoted source policy (source identity/URL plus the effective code-owned Stage 2 dispatch engine, trust/classification and coverage facts) and archives the complete previous calendar payload and source policy before replacing them; if the current `input.xlsx` source configuration drifted since promotion, the refresh is refused unless the operator explicitly passes `--accept-source-policy-change`, so a workbook edit can never silently redefine already-promoted evidence. This operation must not move tournaments, change participants, change approvals/protections/request constraints or absorb new baseline regressions. It deliberately makes existing export/audit evidence stale, so run a fresh export/audit before publication. Do not rerun the full Stage 1-4 planner merely to refresh calendar availability for an already promoted season.
...
One definition of "confirmed booked": only an explicit event-to-tournament association (or an already-valid one) is `confirmed_booked`, and a tournament is `confirmed_not_booked` only from explicit source-supported negative/rejection evidence. Bulk `season reconcile-calendar-bookings` records only evidence: a trustworthy host calendar with no event covering the current canonical slot becomes `ambiguous` (`no_covering_event_for_current_slot`), a lone overlapping event stays `ambiguous` candidate evidence, and multiple overlaps stay `ambiguous`; zero overlap is an observation about that slot, not proof that the booking does not exist elsewhere (it may have moved, or the source may be incomplete). It never promotes mere overlap to a confirmed booking and never infers a negative booking from calendar absence. Confirm the semantic match for an ambiguous candidate with `season confirm-calendar-booking`; `confirm-calendar-booking` is the only path that creates `confirmed_booked`. The booking-status projection enforces the same invariants: a persisted positive evidence record with no currently-valid explicit association (a legacy overlap match, or one whose association was released without rebinding) is surfaced as an ambiguous item requiring review, not as `confirmed_booked`, and a `confirmed_not_booked` record whose reason is current-slot absence is surfaced as ambiguity requiring review, not as `confirmed_not_booked`; the historical evidence record itself is preserved.

When no calendar can be scraped, matched or trusted (a blocked/empty source, a generic overlap, or an explicit club statement that its bookings are made but not reflected in a public calendar), record the decision as an explicit manual assertion with `season booking-set`. It is durable source authority, not a scrape result: it lives in its own decisions key, is revision-bound to the current canonical slot, and is never erased or demoted by a later `season reconcile-calendar-bookings`/`season refresh-calendars`. It projects as `manually_booked`/`manually_not_booked` with `authority=manual_club_confirmation`, distinct from calendar-verified `confirmed_booked`/`confirmed_not_booked`; a `club_wide_interpretation` scope is projected as `authority=manual_club_confirmation_interpretation` and the row retains its `source_scope`. A material canonical change (date/start/duration/host/arena) stales the assertion; re-confirming the new slot with a fresh `--reference`/`--note` replaces the stale assertion directly and keeps the old record as `superseded` audit history (no `--supersede` needed), while changing the conclusion about a still-current slot requires `--supersede` with a reason. A later calendar event that contradicts it surfaces a conflict requiring review instead of silently overwriting, and independent actionable calendar warnings (for example a stale association) stay visible in `follow_up_reasons` without demoting the club confirmation. A positive confirmation requires a traceable `--reference` or `--note`. Use `--reference` for the actual source (email id/date/sender) and `--source-scope club_wide_interpretation` when the assertion is one deliberate per-tournament interpretation of a club-wide statement rather than a fabricated itemized confirmation. A stated source interval that differs from the canonical occupancy (for example a shorter emailed window) is recorded with `--stated-start`/`--stated-end` as follow-up; the canonical occupancy is never silently shrunk, malformed/zero-length/reversed same-day windows are rejected at the command boundary, and overnight windows are not supported. Repeating the same assertion is idempotent; `season booking-clear` revokes an incorrect assertion. Rejection never cancels or deletes the tournament; it stays visible as follow-up. **Never regenerate or reopen a published/sealed season to work around a scrape failure** -- this manual fallback exists precisely so a failed source does not require replanning.
